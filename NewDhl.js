...
let DataOtherPrices= [];
let UrlSetting= base + "&sheet=Setting&tq=" + encodeURIComponent('Select *');
let DataSetting= [];
...
function LoadSetting(){
  ...
  .then(rep => {
      ...
      jsonSetting.table.rows.forEach((rowData9) => {
          const rowSetting = {};
          colzSetting.forEach((ele, ind) => {
              rowSetting[ele] = (rowData9.c[ind] != null) ? rowData9.c[ind].v : '';
          })
...
function ShowResultDiv(BasicPrice,ValueB,Method00,WeightT,LengthT){
  ...
  let Cost0=document.getElementById("Cost0");
  let Cost1=document.getElementById("Cost1");
  let Cost2=document.getElementById("Cost2");
  let MyPer0=document.getElementById("MyPer0");
  let AllTotalPrice=document.getElementById("AllTotalPrice");
  let AllCost=document.getElementById("AllCost");
  if (isNaN(DataSetting[0].Num)==false){   
    for (let indeA = 0; indeA < DataSetting.length; indeA++) {
      if(DataSetting[indeA].Per=="هامش الربح"){
        MyPer0.value= DataSetting[indeA].Amount;
      }    
      if(DataSetting[indeA].Per=="شهر"){
        Cost0.value=DataSetting[indeA].Amount;
      }     
      if(DataSetting[indeA].Per=="شحنة"){
        Cost1.value=DataSetting[indeA].Amount;
      } 
      if(DataSetting[indeA].Per=="كغ"){
        Cost2.value=DataSetting[indeA].Amount * WeightT;
      }  
    }
  };
  AllCost.value=RoundNumbers(parseFloat(TotalPrice.value) + parseFloat(Cost0.value) + parseFloat(Cost1.value) + parseFloat(Cost2.value),2)
  MyPer.value=RoundNumbers(AllCost.value*MyPer0.value,2);
  AllTotalPrice.value=RoundNumbers(parseFloat(TotalPrice.value) + parseFloat(MyPer.value) + parseFloat(Cost0.value) + parseFloat(Cost1.value) + parseFloat(Cost2.value),2);
}
...
function ReCaluclateOthers(){
  ...
  if (isNaN(DataSetting[0].Num)==false){   
    for (let indeA = 0; indeA < DataSetting.length; indeA++) {
      ...
      if(DataSetting[indeA].Per=="كغ"){
        Cost2.value=DataSetting[indeA].Amount * WeightT.value;
      }  
    }
  };
    AllCost.value=RoundNumbers(parseFloat(TotalPrice.value) + parseFloat(Cost0.value) + parseFloat(Cost1.value) + parseFloat(Cost2.value),2);
...
function ReCaluclateOthers1(){
  ...
  AllCost.value=RoundNumbers(parseFloat(TotalPrice.value) + parseFloat(Cost0.value) + parseFloat(Cost1.value) + parseFloat(Cost2.value),2)
    MyPer0.value=RoundNumbers(MyPer.value/AllCost.value,2);
    AllTotalPrice.value=RoundNumbers(parseFloat(TotalPrice.value) + parseFloat(MyPer.value) + parseFloat(Cost0.value) + parseFloat(Cost1.value) + parseFloat(Cost2.value),2);
}
